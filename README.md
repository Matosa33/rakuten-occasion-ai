...
> **État du projet** : pipeline **photo-first complet et fonctionnel**, enrichi par l'**identification raisonnée** (jugement du modèle sur le top-15 ancré, priorité au texte vendeur, ancre prix neuf estimée par IA, fiche exhaustive sourcée, parcours vendeur refondu). L'ensemble est livré : data, retrieval, classifieurs, pricing, explicabilité, API, frontend, MLOps de bout en bout, sécurité, CI/CD. **406 tests verts**, vérifié en conditions réelles sur de vraies photos. Deux chantiers sont volontairement différés et documentés : l'indexation vision SigLIP du catalogue (conditionnée à un test de mesure) et le fine-tuning VLM par QLoRA (conditionnel à un gain mesuré). Détail honnête en bas de page.
...
```
   ...
   titre + attributs JSON              (les questions d'état communes ;
   {marque, modèle, couleur…}           la checklist SPÉCIFIQUE au
                                        type vient après l'identification)
              │
              ▼
   RETRIEVAL ANCRÉ                     ◄── cœur de l'identification
   Arctic Embed → FAISS HNSW               le retriever apporte la CONNAISSANCE
   sur ~4,5 M produits (4 catégories)      (top-15 candidats réels)
              │
              ▼
   IDENTIFICATION RAISONNÉE            ◄── le modèle vision-langage apporte le
   1 appel modèle sur le top-15 (texte)    JUGEMENT : famille + bon candidat
   {famille, candidat choisi, ancre prix    (réordonné) + ancre prix NEUF +
    neuf, produit absent, question}          question discriminante. Ancré :
              │                              jamais hors des 15 fiches.
              ▼
   PRIORITÉ AU TEXTE VENDEUR           ◄── si le vendeur NOMME un produit
   (déterministe, ≥2 mots, ≥50 %)           présent dans les candidats, il fait
              │                              FOI (indépendant du modèle rapide)
              ▼
   ...
   (cascade       description ancrée
    ...
    USD→EUR)
       ...
       └──────┬───────┘
              ▼
   ANNONCE QUALITÉ MARKETPLACE
   fil d'Ariane catégorie · galerie photos · prix + badge état
   ...
   catalogue » · « Prix neuf estimé par IA » (niveau L1.5)
```
...
> du catalogue (priorité au texte vendeur), l'*état général* (dont « Pour pièces / HS »)
> ajuste le prix et le titre, l'*année d'achat* (option) affine la décote par âge. Le
> **type d'objet n'est plus saisi à la main** : il est **dérivé de la catégorie
...
Les artefacts lourds (index FAISS, embeddings, modèles, dumps catalogue) sont **volontairement absents du dépôt** (gitignorés et reconstruits par le pipeline de données). Sans eux, on peut néanmoins **tout vérifier sauf l'inférence live** :
...
Les tests **ne dépendent pas des artefacts lourds** : ils valident la logique (parsing VLM tolérant, anti-path-traversal des uploads, cascade de pricing, cohérence devise, garde-fous hors-distribution, contrats Docker/compose…) avec des mocks et fixtures. La CI GitHub Actions rejoue exactement cela (`.github/workflows/ci.yml`).
...
|---|---|---|
| F1 pondéré classification (FAISS k-NN, vainqueur du benchmark) | **0.954** | `reports/04_classifiers_bench/` |
...
| Métrique | Valeur | Source |
|---|---|---|
| Identification (≥ 50 % des mots-clés retrouvés) | **90.3 %** (rappel moyen **78 %**) | `reports/09_fiche_quality/` |
...
| Produit absent du catalogue détecté | **28 %** | `reports/09_fiche_quality/` |
...
│   ├── exigences_*.md       # Traçabilité des exigences fonctionnelles + couverture
│   └── GLOSSAIRE.md        # Tout le jargon en une ligne
├── src/
│   ├── data/               # Audit → cleaning → split → validation (Pandera)
│   ├── encoders/           # Arctic Embed (+ SigLIP conditionnel)
│   ├── classifiers/        # Modèles benchmark + TF-IDF + fusion
│   ├── retrieval/          # FAISS HNSW + RRF + hors-distribution + Akinator
│   ├── vlm/                # Extraction photo + validateur visuel + identification raisonnée
│   ├── llm/                # Prompts + rédacteur RAG ancré
│   ├── pricing/            # Cascade algorithmique transparente (L1→L1.5→L2→L3→L4)
│   ├── explain/            # SHAP + t-SNE + Model Cards
│   ├── api/                # FastAPI + SSE + uploads + auth JWT + priorité au texte vendeur
│   ├── auth/               # JWT HS256 (bcrypt)
│   ├── mlops/              # MLflow / promotion champion-challenger
│   ├── serving/            # BentoML + assemblage de la fiche exhaustive sourcée (listing_fill)
│   ├── monitoring/         # Drift Evidently + push metrics
│   └── observability/      # structlog + Prometheus
├── frontend/               # React 19 (flow unitaire « Un objet » + mode batch « Déménagement »)
├── scripts/                # mesure_qualite_fiche.py (protocole panel réel) …
...
- **Zéro secret dans le code** : tout en `.env` (gitignoré). `.env.example` ne contient que des gabarits.
...
- **Durcissement issu d'une revue de sécurité adverse** (cinq dimensions, corrections appliquées) : bornes Pydantic sur les prix (`gt=0`, `le=1e6`, `allow_inf_nan=False`) et sur les listes/textes d'entrée (anti-déni de service du prompt LLM) ; **texte vendeur délimité comme donnée non fiable** dans le prompt (anti prompt-injection : il n'est jamais interprété comme une consigne).
...
- **Indexation vision SigLIP du catalogue** : l'entrée photo fonctionne aujourd'hui via **extraction VLM** (le modèle lit la photo → titre/attributs → retrieval texte). L'indexation visuelle directe du catalogue (recherche image vers image sur 4,5 M produits) exige un téléchargement CDN de plusieurs dizaines d'heures ; elle est **conditionnée à un test de mesure d'une nuit** (preuve de concept `reports/11_poc_photo/`) qui décidera GO/NO-GO sur preuve, pas sur intuition.
- **Fine-tuning VLM QLoRA** : déclenché **uniquement** si un benchmark montre un gain > +0.05 F1 macro face à l'approche ancrée sans spécialisation. Non déclenché à ce jour faute de bénéfice démontré : la sur-ingénierie est un anti-pattern.

### Limites honnêtes de l'identification raisonnée
