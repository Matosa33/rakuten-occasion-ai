...
- **`TRACKING_URI` paramétrable** : `MLFLOW_TRACKING_URI` (env) → serveur **Postgres+MinIO** en
  Compose (D-027), sinon **SQLite local** `mlflow.db` en dev hôte. Détection auto du backend HTTP
...
- **R5 « MLflow obligatoire »** : chaque classifieur (M1-M6) + pricing logue son run.
...
- **Double backend réconcilié (C32.1)** : le serveur conteneur (`:5000`, Postgres+MinIO) a été
  aligné en **MLflow 3.x** (il était bloqué en 2.22.5 vs client 3.x) ; il contient désormais
...
**Solide :**
- ✅ **Tous les entraînements tracés** (R5), métriques riches, comparables, **+ commit git** →
  reproductibles.
