...
- `00_vue_ensemble.md` : le système entier en un seul coup d'œil. Le trajet complet d'une photo jusqu'à l'annonce, le principe « ancré » qui empêche d'inventer, et la façon dont le prix est calculé. C'est la lecture d'introduction la plus importante. Ce document décrit aussi le mode d'identification dit « raisonné », où le moteur de recherche apporte d'abord les quinze produits réels les plus proches (la connaissance) puis le modèle ne fait que choisir le bon parmi eux et l'expliquer (le jugement) : il ne peut jamais proposer un produit absent de cette liste. On y trouve enfin la cascade de prix complète, du plus fiable au plus prudent, y compris un niveau intermédiaire où le prix neuf de référence est une estimation honnêtement étiquetée de l'IA quand le prix exact manque au catalogue, le tout suivi des résultats mesurés sur un panel réel de produits photographiés.
...
- `05_vlm_llm_grounded.md` : les deux intelligences artificielles génératives du projet. La première « voit » : elle lit la photo et en déduit le produit probable. La seconde « rédige » : elle écrit l'annonce. Les deux sont volontairement bridées par des faits réels pour les empêcher d'inventer. C'est le garde-fou anti-invention, présent du début à la fin du traitement. C'est aussi ici qu'intervient l'identification « raisonnée » : en un seul appel, le modèle qui voit reçoit les photos du vendeur et les quinze fiches candidates résumées en texte, puis choisit la bonne, signale si le produit est absent du catalogue plutôt que de se rabattre sur un sosie, propose un prix neuf de référence et, au besoin, la question qui lèverait le doute. Toutes ses réponses doivent citer leur source (une fiche réelle ou la photo), faute de quoi elles sont rejetées, et toute défaillance fait simplement revenir au classement du moteur de recherche sans jamais bloquer ni produire une fiche vide.

- `06_pricing.md` : le calcul du prix conseillé. Plutôt qu'un chiffre opaque sorti d'une boîte noire, le système propose un prix accompagné d'une fourchette, d'un niveau de confiance et d'une explication lisible, pour que le vendeur comprenne pourquoi ce prix lui est proposé. Le prix descend une cascade de niveaux du plus fiable au plus prudent : on part du prix réel du produit identifié dans le catalogue, puis, si ce prix manque, d'un prix neuf de référence estimé par l'IA et clairement étiqueté comme tel, puis seulement du prix médian des produits voisins, puis de la catégorie, et enfin d'une saisie manuelle. Le point important est que seul ce prix neuf de référence intermédiaire est une estimation : la décote appliquée ensuite selon l'état (du neuf au « pour pièces ») et l'âge reste, elle, entièrement déterministe et explicable. Le document détaille aussi les garde-fous qui empêchent un prix absurde, comme une médiane de voisins effondrée par des accessoires ou une donnée de catalogue manifestement corrompue.
...
- `11_observabilite.md` : la capacité de voir si l'application va bien une fois en service. Combien de requêtes elle reçoit, à quelle vitesse elle répond, combien d'erreurs surviennent, et comment retracer pas à pas ce qui s'est passé pour une requête donnée grâce à un identifiant unique attaché à chacune. À ces mesures techniques s'ajoutent désormais des traces métier lisibles : pour chaque identification on enregistre l'issue réelle (produit retenu, score, catégorie, présence d'une question à poser, produit absent du catalogue, ancre de prix, temps passé dans chaque appel au modèle) et, pour chaque prix, le niveau de cascade effectivement utilisé. On peut ainsi lire ce que fait vraiment le système, par exemple à quelle fréquence un produit est absent du catalogue ou quel niveau de prix sert le plus souvent.
...
- `14_api_orchestration.md` : le chef d'orchestre du système. C'est la couche qui reçoit les demandes, charge les modèles, enchaîne dans le bon ordre la recherche du produit, la lecture de la photo, la levée de doute, le calcul du prix et la rédaction, puis renvoie une réponse propre. Sans elle, les briques d'intelligence artificielle resteraient des fichiers isolés. C'est aussi cette couche qui pilote l'identification raisonnée : elle remonte en tête le produit choisi parmi les candidats afin que la fiche, le prix et la validation portent sur le bon produit, et elle traite comme prioritaire le texte du vendeur lorsqu'il nomme sans ambiguïté un produit présent dans la liste, de façon déterministe et indépendante du modèle.

- `15_frontend_ux.md` : l'interface que le vendeur utilise vraiment dans son navigateur, pour photographier, vérifier et publier. C'est l'endroit où tout le traitement automatique devient un produit réellement utilisable, et où l'ergonomie décide si le vendeur va jusqu'au bout. Le parcours a été simplifié pour s'appuyer au maximum sur l'identification automatique : le vendeur ne choisit plus lui-même le type d'objet, il est déduit de la catégorie reconnue, ce qui permet d'afficher au bon moment la bonne liste de questions d'état (batterie et écran pour un téléphone, clavier pour un ordinateur, manettes pour une console). Quand l'IA est sûre du produit, la fiche s'affiche directement, avec toujours un bouton pour consulter les autres résultats si ce n'est pas le bon ; quand le modèle exact reste ambigu, un bandeau « modèle à confirmer » invite à préciser plutôt que d'afficher un pourcentage de confiance trompeur, et un bandeau distinct signale un produit estimé absent du catalogue. L'état « pour pièces / hors service » et une année d'achat facultative ont aussi été ajoutés, car ils pèsent sur le prix conseillé.
...
- `17_etude_chaine_valeur_photo.md` : l'étude scientifique qui prouve, chiffres et tests statistiques à l'appui, la valeur de chaque maillon de la chaîne. Elle démontre sur 93 annonces réelles que la photo est le signal décisif (mieux que le texte du vendeur, et plusieurs photos mieux qu'une seule), que les informations saisies par le vendeur n'ajoutent rien une fois les photos disponibles, et quel modèle de lecture d'image a été retenu après comparaison. Le même document mesure désormais la qualité de la fiche produite de bout en bout : un panel réel de 94 dossiers (les noms de dossiers servant de vérité-terrain, dont un sans photo n'est pas évalué, soit 93 produits réellement mesurés) est rejoué par l'application, photo seule sans texte du vendeur. Sur ce panel, le bon produit est identifié dans 90,3 % des cas et la fiche est remplie à 0,84 en moyenne (0,83 en médiane). Cette mesure est produite par le script `scripts/mesure_qualite_fiche.py`, qui écrit le rapport reproductible dans `reports/09_fiche_quality/` (fichiers `mesure_fiche.json` et `mesure_fiche.md`, un résultat chiffré défendable plutôt que des exemples choisis). C'est la pièce maîtresse de la démonstration.
...
Notre système ne devine pas un produit, il le retrouve dans un catalogue réel puis rédige l'annonce à partir de ces faits vérifiés. La photo suffit à démarrer, l'humain valide toujours le résultat, le prix s'explique en clair, et toute la chaîne, des données propres jusqu'à la surveillance en service, est mesurée et reproductible. Mesurée jusqu'au bout : sur un panel réel de produits photographiés, photo seule, le bon produit est identifié dans 90,3 % des cas et la fiche est remplie à 0,84 en moyenne, avec un système qui préfère signaler qu'un produit est absent du catalogue plutôt que d'inventer un sosie.
