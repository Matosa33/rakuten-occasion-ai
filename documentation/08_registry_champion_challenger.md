...
3. **Promouvoir seulement si `challenger ≥ champion + ε`** (ε = petit seuil → on ne bouge pas
   pour du bruit).
4. Cas « pas de nouveau challenger » (D-023 : artefact non poussé → pas de nouvelle version) →
...
- **C32.1** : le registry est désormais aussi peuplé dans le serveur conteneur (`:5000`) après
